...
export default function Header({ dark, setDark }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={` md:scale-100 scale-0 py-3 rounded-full  text-[1.1rem] flex items-center  md:justify-between 
      ...
      <div className=" flex items-center gap-1 md:gap-4">
        <button className=" px-5 py-[0.3rem] rounded-md bg-emerald-800 tracking-widest font-[500] hover:bg-transparent hover:text-emerald-600 border-[0.001rem] hover:border-emerald-600  border-emerald-800 duration-500 text-[0.9rem]  text-zinc-100 " onClick={()=>supabaseContext.setLoginPage(true)}>Login</button>
      <div
      ...
      <div className="text-[1.5rem]  lg:hidden text-zinc-300 w-10 h-10 rounded-full hover:text-zinc-100 hover:bg-emerald-700 grid place-items-center duration-500 cursor-pointer" onClick={()=>supabaseContext.setSidebar(true)}>
        <HiMenuAlt3/>
      </div>
      </div>
    </div>
